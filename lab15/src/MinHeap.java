...
public class MinHeap<E extends Comparable<E>> {
    ...
    private int min(int index1, int index2) {
        ...
        if (contents.get(index1) == null || contents.get(index2) == null){
            return contents.get(index1) == null ? index2 : index1;
        }
        return contents.get(index1).compareTo(contents.get(index2)) <= 0 ? index1 : index2;
    }
    ...
    private void bubbleDown(int index) {
        // TODO: YOUR CODE HERE
        if (getLeftOf(index) != -1) {
            if (getRightOf(index) != -1) {
                while (contents.get(index).compareTo(contents.get(min(getLeftOf(index),getRightOf(index)))) > 0) {
                    int temp = min(getLeftOf(index), getRightOf(index));
                    swap(temp, index);
                    bubbleDown(temp);
                }
            } else {
                while (contents.get(index).compareTo(contents.get(getLeftOf(index))) > 0) {
