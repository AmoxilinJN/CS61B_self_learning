...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba> {
    ...
    public String longestName() {
        // TODO: YOUR CODE HERE
        if (this.root != null){
            return root.longestNameHelper(root.name);
        }
        return null;
    }
    ...
    public static class Amoeba {
        ...
        public String longestNameHelper(String maxName){
            for (Amoeba a : this.children){
                if (maxName.length() < a.name.length()){
                    maxName = a.name;
                }
                maxName = a.longestNameHelper(maxName);
            }
            return maxName;
        }
    ...
    public class AmoebaBFSIterator implements Iterator<Amoeba> {
        ...
        public Amoeba next() {
            ...
            Amoeba temp = fringe.removeFirst();
            int i = 0;
            while (i < temp.children.size()){
                fringe.addLast(temp.children.get(i));
                i++;
            }
            return temp;
        }
