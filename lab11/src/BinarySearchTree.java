public class BinarySearchTree<T extends Comparable<T>> extends BinaryTree<T> {
    ...
    private boolean containsHelper(TreeNode<T> temp,T key){
        if (temp.item.equals(key)){
            ...
            return containsHelper(temp.left,key);
        } else {
            if (temp.right == null){
                return false;
            }
            return containsHelper(temp.right,key);
        }
    }
    ...
    private void addHelper(TreeNode<T> temp,T key){
        if (temp.item.equals(key)){
            return;
        } else if (temp.item.compareTo(key) > 0){
            if (temp.left == null){
