...
public class DistributionSorts {
    ...
    private static void countingSortOnDigit(int[] arr, int digit) {
        ...
        start[0] = 0;
        for (int i = 0; i < arr.length; i++) {
            count[arr[i] % (int) Math.pow(10,digit + 1) / (int) Math.pow(10,digit)]++;
        }
        for (int i = 1; i < 10; i++) {
            start[i] += start[i - 1] + count[i - 1];
        }
        for (int i = 0; i < arr.length; i++) {
            result[start[arr[i] % (int) Math.pow(10,digit + 1) / (int) Math.pow(10,digit)]] = arr[i];
            start[arr[i] % (int) Math.pow(10,digit + 1) / (int) Math.pow(10,digit)]++;
        }
        for (int i = 0; i < result.length; i++) {
            arr[i] = result[i];
        }
